...
class bytemaster_api:
    ...
    def login(self):
        token_json_contents=open("./token.json", "r+t")
        data_token=json.load(token_json_contents)
               
        data=conf().Conf()
        
        if ENTORNO == "curso":
            self.user = USER_ANE_TEST
            self.password = PW_ANE_TEST
            fecha_token=datetime.strptime(data_token["fecha_curso"],"%Y-%m-%d %X")
            token_fichero = data_token["token_curso"]
        else:
            self.user = USER_ANE
            self.password = PW_ANE
            fecha_token=datetime.strptime(data_token["fecha_prod"],"%Y-%m-%d %X")
            token_fichero = data_token["token_prod"]

        ahora=datetime.now()
        diferencia = ahora-fecha_token
        
        if diferencia.total_seconds()/3600<=12:
            self.token=token_fichero
            aut ='Bearer '+token_fichero
        else:        
            url_=self.url+"Autentificacion/Validacion"
            json_={"usuario": self.user, "clave": self.password}
            self.token = requests.post(url_,json=json_, headers=self.headers)
            jsonstr=self.token.json()
            aut ='Bearer '+jsonstr["token"]

            #Modificar el fichero para que guarde el token
            if data["entorno"]== "curso":
                data_token["token_curso"]=jsonstr["token"]
                data_token["fecha_curso"]=datetime.now().strftime("%Y-%m-%d %X")
            else:
                data_token["token_prod"]=jsonstr["token"]
                data_token["fecha_prod"]=datetime.now().strftime("%Y-%m-%d %X")

            token_json_contents.close()
            token_json_contents=open("./token.json", "wt")
            token_json_contents.write(json.dumps(data_token, indent = 6))
            token_json_contents.close()
        
        self.headers={'content-type':'application/json', 'encoding': 'charset=utf-8','Authorization': aut}
      
    def __in_clause__ (self, refs=[]):
        in_clause="("
        for x in range(len(refs)-1):
            if x==0:
                in_clause+=str(refs[x]["iref"])
            else:
                in_clause+=", "+str(refs[x]["iref"])

        in_clause+=")"
        return in_clause
    def fecha_iso (fecha_entrada="", formato="YYYYmmDD"):
        fecha=datetime.strptime(fecha_entrada,formato)
        return fecha.strftime("yyyy-MM-ddTHH:MM.000Z")
    ...
    def post_expediente(self, data_json):
        ...
        try:
            ...
            #respuesta= requests.post(url_,headers=self.headers,json=data_json)
    ...
    def post_Entrada(self, data_json):
        ...
        try:
            ...
            print (fallo)     
    def post_Entrada_linea(self,id=0 ,data_json={}):
        url_=self.url+'entrada/'+str(id)+"/lineas/"
    ...
    def post_Entrada_etiqueta(self, data_json={}, ialment=0):
        ...
        try:
            ...
            print (fallo)       
    def put_Entrada(self, data_json={}, ialment=0):
    ...
    def post_expediente_hito(self, data_json,id_expediente):
        ...
        try:
            ...
            print (fallo)
    def post_albaran_vinculos(self, id, data_json):
        url_=self.url+'Albaran/'+str(id)+"/vinculos"
        ...
        try:
            ...
            print (fallo)   
    def post_albaran_vinculos_tracking(self, id, data_json):
        url_=self.url+'Albaran/'+str(id)+"/tracking"
    ...
    def put_partida(self,ipda=0, data_json={}):
    ...
    def post_partida_vinculos(self, ipda, data_json):
        ...
        try:
            ...
            print (fallo)   
    def post_partida_vinculos_tracking(self, id, data_json):
        url_=self.url+'Partida/'+str(id)+"/tracking"
        ...
        try:
            ...
            print (fallo)   
    def post_partida_lineas_mercancia(self, id, data_json):
        url_=self.url+'Partida/'+str(id)+"/lineas"
        ...
        try:
            ...
            print (fallo)   
    def post_partida_etiqueta(self, id, data_json):
        url_=self.url+'Partida/'+str(id)+"/etiquetas"
    ...
    def referencias (self, propietario):
        ...
        try:
            ...
            while x<=total or flag:
                ...
                in_refs.append(self.__in_clause__(refs[x_1:x]))
                x_1=x
                if x+50<total:
                    x=x+50
                elif x==total:
            ...
            for in_iref in in_refs:
                ...
                if propietario==None: #Referencias sin propietarios
                    ient_query=""
            ...
            #"select COUNT(1) as cuenta from laxref WHERE FMOD>'2021-01-01' OR falt>'2021-01-01'"
    ...
    def linea_Alb_Salida_post(self,id, linea_json={}):
